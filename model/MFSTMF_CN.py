...
'''
# @Author  ：kinnisoy
# @Date    ：03.27 027 下午 18:27 
# @File    ：MFSTMF_CN.py
# @Description : MY MODEL： Map Free Spatial-Temporal model for Motion Forecasting
'''
import math
import numpy as np
import os
...
# root_path =  "/home/code/dataset/argoverse2"#

class MFSTMF(pl.LightningModule):
    def __init__(self, args):
        super(MFSTMF, self).__init__()
        self.args = args
        self.save_hyperparameters()
        # input embedding encoder (3,64)
        self.encoder_lstm = EncoderLstm(self.args)
        # Temporal - Spatial Block
        self.temporal_att1 = MultiheadSelfAttention(self.args)  #(64,64)
        self.spatial_gnn1 = AgentGnn(self.args)   #(64,)
        # Spatial - Temporal Block
        self.spatial_gnn2 = AgentGnn(self.args)
        self.temporal_att2 = MultiheadSelfAttention(self.args)
        # decoder residual
    ...
    def forward(self, batch):
        ...
        # ENCODER:
        out_encoder_lstm = self.encoder_lstm(displ_cat, agents_per_sample)  # (N,64)agent_per_sample not used
        # TS BLOCK:
        temproal_out_1 = self.temporal_att1(out_encoder_lstm, agents_per_sample)
        out_temproal_1 = torch.cat(temproal_out_1)
        out_temproal_spatial = self.spatial_gnn1(out_temproal_1,centers_cat, agents_per_sample)

        # ST BLOCK:
        spatial_out_2 = self.spatial_gnn2(
            out_encoder_lstm, centers_cat, agents_per_sample)   # (N,64)
        out_temproal_2 = self.temporal_att2(
            spatial_out_2, agents_per_sample)  # ()
        out_spatial_temproal  = torch.cat( out_temproal_2)
        #list(2)(22,64)(29,64)
        # fusion
        fusion = out_spatial_temproal+out_temproal_spatial
        fusion = self.split_2_batch_sz(fusion,agents_per_sample,self.args.latent_size)
        # Todo: split the fusion to a list matched the  batch num ,eg(batch_sz,hidden) Finished Above.
        fusion = torch.stack([x[0] for x in fusion])     #(2,64)--->(2,6,120)
        out_linear = self.decoder_residual(fusion, self.is_frozen)
    ...
    def prediction_loss(self, preds, gts):
        ...
        gt_target = torch.repeat_interleave(gt_target, num_mods, dim=0)  # repeat k time for multi-model pred
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        self.log("loss_train", loss / len(out)) # , batch_size=self.args.batch_size
        return loss
    ...
    def validation_step(self, val_batch, batch_idx):
        ...
        self.log("loss_val", loss / len(out), batch_size=self.args.batch_size)
    ...
    def split_2_batch_sz(self,fusion_in, agents_per_sample,latent_size):
        out_batch = []
        if len(agents_per_sample) > 1:
            max_agents = max(agents_per_sample)

            padded_fusion_in = torch.zeros(
                (len(agents_per_sample), max_agents, latent_size), device=fusion_in[0].device)
            mask = torch.arange(max_agents) < torch.tensor(
                agents_per_sample)[:, None]  # (2,29)

            padded_fusion_in[mask] = fusion_in
            out_batch = [x[0:agents_per_sample[i]] for i, x in enumerate(padded_fusion_in)]
        else:
            out_batch.append(fusion_in)
...
class MultiheadSelfAttention(nn.Module):
    ...
    def forward(self, att_in, agents_per_sample):
        ...
        if len(agents_per_sample) > 1:
            ...
            padded_att_in[mask] = att_in.type(torch.float)
...
class Linear(nn.Module):
    def __init__(self, n_in, n_out, norm='GN', ng=32, act=True):
        ...
        if norm == 'GN':
            self.norm = nn.GroupNorm(math.gcd(ng, n_out), n_out)
        elif norm == 'BN':
