...
class ArgoCSVDataset(torch.utils.data.Dataset):
    def __init__(self, input_folder, input_preprocessed, args):
        ...
        if args.use_preprocessed:
            ...
            self.maps = glob.glob(f"{input_folder}/**/*.json")
            if args.reduce_dataset_size > 0:
                self.files = self.files[:args.reduce_dataset_size]
                self.maps = self.maps[:args.reduce_dataset_size]
    ...
    def __getitem__(self, idx):
        ...
        if self.args.use_preprocessed:
            ...
            # self.argo_reader = self.argo_reader.extract_data(self.files[idx],self.maps[idx])
            # self.argo_reader["graph"] = self.argo_reader.extract_map(self.maps[idx])
            return self.argo_reader.extract_data(self.files[idx],self.maps[idx])
