...
'''
# @Author  ：kinnisoy
# @Date    ：03.30 030 上午 11:54 
# @File    ：visualize_scenario.py
# @Description : rebuild the repo to viz the prediction value to generate the video.
'''
...
_DEFAULT_ACTOR_COLOR: Final[str] = "#d0cee2"
_FOCAL_AGENT_HIS_COLOR: Final[str] = "#ECA154"
_FOCAL_AGENT_GT_COLOR: Final[str] = "#6ea06f"
_FOCAL_AGENT_PREDICTION_COLOR: Final[str] = "#007672"
_AV_COLOR: Final[str] = "#d0cee2"
...
temp_path = root_path+ "/dataset/argoverse2/to_viz/"
...
def visualize_scenario(scenario: ArgoverseScenario, scenario_static_map: ArgoverseStaticMap, save_path: Path) -> None:
    ...
    for timestep in range(_OBS_DURATION_TIMESTEPS + _PRED_DURATION_TIMESTEPS):
        ...
        # Todo: if timesteps>50 show the prediction
        if timestep >50:
            scenario_id = scenario.scenario_id
            pred_trajs_path = sorted(Path(temp_path + f"/{scenario_id}/").rglob("*.npz"))[0]

            pred_trajs = np.load(str(pred_trajs_path))
            if(show_all):
                for i in range(K_pred):
                    index = f"arr_{i}"
                    i_traj = pred_trajs[index]
                    _plot_polylines([i_traj], color=_FOCAL_AGENT_PREDICTION_COLOR, line_width=2)
            else:
                if timestep <= 120 - K_pred:
                    for i in range(K_pred):
                        index = f"arr_{i}"
                        i_traj = pred_trajs[index][:timestep - 50 + 5]
                        _plot_polylines([i_traj], color=_FOCAL_AGENT_PREDICTION_COLOR, line_width=2)
                else:
                    for i in range(K_pred):
                        index = f"arr_{i}"
...
def plot_scenario(scenario: ArgoverseScenario, scenario_static_map: ArgoverseStaticMap, save_path: Path) -> None:
    # plt.clf()
    plot_bounds: _PlotBounds = (0, 0, 0, 0)
    scenario_id = scenario.scenario_id
    pred_trajs_path = sorted(Path(temp_path + f"/{scenario_id}/").rglob("*.npz"))[0]
    ...
    cur_plot_bounds = plot_actor_real_tracks(ax, scenario)


    for i in range(K_pred):
        index = f"arr_{i}"
        i_traj = pred_trajs[index]
        _plot_polylines([i_traj], color=_FOCAL_AGENT_PREDICTION_COLOR, line_width=1)
        # Plot the end marker for forcasted trajectories
        plt.arrow(
            i_traj[-2, 0],
            i_traj[-2, 1],
            i_traj[-1, 0] - i_traj[-2, 0],
            i_traj[-1, 1] - i_traj[-2, 1],
            color="#007672",
    ...
    # plt.axis('off')
    plt.savefig(str(save_path.parents[0] / f"{save_path}.png"), dpi=300)
    plt.close()
...
def     plot_actor_real_tracks(ax: plt.Axes, scenario: ArgoverseScenario) -> Optional[_PlotBounds]:
    # 将真实轨迹进行绘制出来
    track_bounds = None
    for track in scenario.tracks:
        # Get actor trajectory and heading history and future 3s.
        ...
        past_trajectory = actor_trajectory[:50]
        future_trajectory = actor_trajectory[50:]
        past_heading = actor_headings[:50]
        future_heading = actor_headings[50:]
        ...
        if track.category == TrackCategory.FOCAL_TRACK:
            ...
            track_color = _FOCAL_AGENT_HIS_COLOR
            # Todo： Plot polyline for focal agent GROUND TRUTH location.
            _plot_polylines([past_trajectory], style= "-", line_width=1.0, alpha=1.0, color=track_color,)
            _plot_polylines([future_trajectory], style="-", line_width=1.0, alpha=1.0, color="r", )

            ## Plot the end marker for history
            plt.arrow(
                past_trajectory[-2, 0],
                past_trajectory[-2, 1],
                past_trajectory[-1, 0] - past_trajectory[-2, 0],
                past_trajectory[-1, 1] - past_trajectory[-2, 1],
                color=track_color,
            ...
            plt.arrow(
                future_trajectory[-2, 0],
                future_trajectory[-2, 1],
                future_trajectory[-1, 0] - future_trajectory[-2, 0],
                future_trajectory[-1, 1] - future_trajectory[-2, 1],
                color="r",  # #d33e4c
                label="Ground Truth",
                alpha=1,
                linewidth=2.0,
...
def _plot_actor_tracks(ax: plt.Axes, scenario: ArgoverseScenario, timestep: int) -> Optional[_PlotBounds]:
    ...
    """
    track_bounds = None
    k_len = 5 # the length of pre show.
    for track in scenario.tracks:
        # Get timesteps for which actor data is valid (HISTORY)
        ...
        # Get actor trajectory and heading history and future 3s.
        actor_trajectory: NDArrayFloat = np.array(
            [list(object_state.position) for object_state in track.object_states if
             object_state.timestep <= timestep]
        )
        actor_headings: NDArrayFloat = np.array(
        ...
        if(show_all):
            actor_fu_timesteps: NDArrayInt = np.array(
                [object_state.timestep for object_state in track.object_states if timestep <= object_state.timestep]
            )
            if actor_fu_timesteps.shape[0] < 1:  # or actor_fu_timesteps[-1] != 110
                continue

            # Get actor trajectory and heading history and future 3s.
            actor_fu_trajectory: NDArrayFloat = np.array(
                [list(object_state.position) for object_state in track.object_states if
                 timestep <= object_state.timestep]
            )
            actor_fu_headings: NDArrayFloat = np.array(
                [object_state.heading for object_state in track.object_states if timestep <= object_state.timestep]
            )
        else:
            if 50 <= timestep <= 100:
                actor_fu_timesteps: NDArrayInt = np.array(
                    [object_state.timestep for object_state in track.object_states if
                     timestep <= object_state.timestep <= timestep + k_len and timestep + k_len <= 110]
                )
                if actor_fu_timesteps.shape[0] < 1 or actor_fu_timesteps[-1] != timestep + k_len:
                    continue
                ...
                actor_fu_headings: NDArrayFloat = np.array(
                    [object_state.heading for object_state in track.object_states if
                     timestep <= object_state.timestep <= timestep + k_len and timestep + k_len <= 110]
                )
            elif timestep >= 100:
                actor_fu_timesteps: NDArrayInt = np.array(
        ...
        if track.category == TrackCategory.FOCAL_TRACK:
            ...
            track_color = _FOCAL_AGENT_HIS_COLOR
            # Todo： Plot polyline for focal agent GROUND TRUTH location.
            _plot_traj_polylines([actor_trajectory], color_his=track_color, color_pre= _FOCAL_AGENT_GT_COLOR,line_width=2,timesteps= timestep)
            if timestep >50:
                _plot_polylines([actor_fu_trajectory],color=_FOCAL_AGENT_GT_COLOR,line_width=1)
            # _plot_polylines([actor_trajectory], color=_FOCAL_AGENT_PREDICTION_COLOR, line_width=2)
        elif track.track_id == "AV":
            track_color = _AV_COLOR
        elif track.object_type in _STATIC_OBJECT_TYPES:
            continue
...
def _plot_traj_polylines(
    ...
    *,
    ...
    for polyline in polylines:
        if timesteps >50 :
            plt.plot(polyline[:50, 0], polyline[:50, 1], style, linewidth=line_width, color=color_his, alpha=alpha)
            plt.plot(polyline[50:, 0], polyline[50:, 1], style, linewidth=line_width, color=color_pre, alpha=alpha)
        else:
            plt.plot(polyline[:, 0], polyline[:, 1], style, linewidth=line_width, color=color_his, alpha=alpha)
